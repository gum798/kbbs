...
enum AuthenticationError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .lockScreenUnlockFailed:
            return "KakaoTalk lock screen could not be unlocked. The saved lock password was discarded; kbbs will ask for it again on the next run."
        case .lockPasswordUnavailable:
            return "KakaoTalk is locked and no password is saved. Run `kbbs auth login --auto` in a terminal to unlock and save it."
        case .lockRejectedAccountPassword:
            return "KakaoTalk rejected the saved account password at the lock screen. Run `kbbs auth login` to save your current password, then retry."
        }
    }
}
...
final class KakaoTalkAuthenticator {
    ...
    private func resolveLockPasscode(from store: CredentialStore) throws -> LockPasscode {
        ...
        // Background callers (watch, cron) have no terminal to read a secret

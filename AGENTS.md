...
.build/debug/kbbs [command]    # Run debug build
.build/release/kbbs [command]  # Run release build
```
...
```bash
.build/debug/kbbs status --verbose           # Check accessibility & app status
.build/debug/kbbs inspect --depth 5          # Inspect UI hierarchy
.build/debug/kbbs chats --verbose --limit 20 # List chats
.build/debug/kbbs send "Name" "msg" --dry-run # Test send without sending
.build/debug/kbbs read "Chat" --limit 50     # Read messages
```

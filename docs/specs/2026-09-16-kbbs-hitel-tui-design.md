...
## 13. 실측으로 확인된 것 (2026-09-16, M1 구현 중)

설계는 전부 코드 읽기에 기반했다. 실제로 돌려보며 확인되거나 뒤집힌 것들.

### 확인됨

- **무제한 BFS 는 진짜로 멈춘다.** 스카웃이 경고한 `findFirst(...)` 계열의
  maxNodes 없는 순회를 카카오톡 트리에 돌렸더니 **7분 넘게 끝나지 않아 강제
  종료**했다. 예산을 준 `findAll(where:limit:maxNodes:)` 로 바꾸니 1초.
  → 이 저장소에서 무예산 탐색은 금지. 예외 없음.
- **AX 경로 캐시를 지워도 스크레이퍼는 그대로 동작한다.** 30줄 no-op 심으로
  치환한 뒤 `ChatListScanner` / `MessageContextResolver` / `TranscriptReader`
  를 한 줄도 고치지 않고 빌드·동작. 421줄의 비동기화 가변 상태가 사라졌다.
- **한글 폭 계산은 교차검증 통과.** 24행 × 80칸을 파이썬 `east_asian_width`
  독립 구현으로 검산. `ㅋㅋㅋ` 가 호환 자모(U+314B)라 음절 범위만 아는 표로는
  틀린다는 것도 테스트로 고정했다.

### 새로 발견된 결함

- **`findWindow(title:)` 은 제목만 본다.** 카카오톡이 실행 중이지만 창이 전부
  닫혀 있으면 AX 트리에 메뉴바만 남고, 「카카오톡」이라는 제목을 가진 유일한
  요소는 **애플리케이션 요소**다. `chatListWindow` 가 그것을 반환하고 부트
  사다리는 "대화목록 창 확인" 이라고 보고한 뒤 0개를 읽었다 — 두 줄이 서로
  모순된다. 이제 `role == AXWindow` 를 검증한다. (kmsg 에도 있는 문제지만
  kmsg 는 수정하지 않는다.)

### M1 실물 렌더 (2026-09-16, 카카오톡 창 60개 목록)

설계도 목업도 잡아내지 못한 것들. 전부 진짜 대화방 목록을 한 번 그려서 나왔다.

- **미리보기에 개행이 들어있다.** `[모빙]\n고객님 안녕하세요!` 가 한 방의 미리보기
  한 개다. 제어문자는 폭 0으로 세지므로 행은 "꽉 찼다"고 측정된 뒤 그대로 나가고,
  터미널이 거기서 줄을 바꾼다. 60개 중 4개 행이 이렇게 상자를 무너뜨렸다.
  → `Width.oneLine` 으로 `Room` 진입 시점에 한 줄로 만들고, `Frame.fit` 은 남은
  제어문자를 공백 한 칸으로 치환한다 (칸 수까지 정직하게 1칸으로 센다).
- **시각은 `오후 1:21` 로 온다.** `isTimeLikeValue` 는 맨 H:MM 만 알아서 오늘 온 방은
  전부 시각이 비었고, 어제/N일 행만 채워져 있었다. 게다가 `오후 1:21` 은 9칸이라
  5칸 컬럼에 애초에 안 들어간다. → `compactTime` 으로 24시간 `13:21` 로 줄인다.
- **BMP 이모지가 폭 표에 없었다.** 방 이름 `윤지원✨` (U+2728, Dingbats) 이 1칸으로
  세어져 그 행만 81칸이 됐다. 1F300 위쪽만 이모지로 알고 있었다. → Emoji_Presentation
  BMP 범위 추가, U+FE0F 가 붙은 클러스터는 2칸.
- **폭 표는 이진탐색이라 정렬이 동작 조건이다.** 범위를 잘못된 자리에 끼워 넣자
  104개 중 17개가 깨졌다. → 표를 손으로 정렬하는 대신 `sorted(...)` 로 감쌌다.

**측정:** 60개 방 목록 1회 스캔 = **약 8.6초** (cold, `--limit 60`). 설계가 잡은
...
### A3 실측 — 3초 주기는 불가능하다 (2026-09-16)

`--room "…" --once` 로 창이 열려 있는 실제 대화방을 읽어서 쟀다.

| | |
|---|---|
| 대화방 열기 (`MessageContextResolver` 1회) | **4.65초** |
| 따뜻한 읽기 (`readSnapshot(from: context …)`) | **3.0 ~ 8.6초** |

`--limit` 을 10/20/40/60 으로 바꿔도 2.96 / 8.58 / 6.17 / 5.97 초로 **한계값과 상관이
없다.** 이유는 `collectTranscriptRows` 의 바닥값이다: `targetRowCount = max(limit*4, 50)`,
`recentWindow = max(limit*6, 80)` 이라 limit 10 이든 60 이든 이 방의 62행을 전부 읽고,
행마다 `findAll(role: kAXStaticTextRole …)` 과 frame 조회가 돈다. 비용을 지배하는 것은
메시지 개수가 아니라 AX 왕복 횟수다.

**설계가 깨진 지점:**

- **3초 폴링은 성립하지 않는다.** 읽기 한 번이 3~9초다. 3초 주기로 걸면 읽기가 끝나기
  전에 다음 읽기가 예약된다.
- **인라인 호출은 못 쓴다.** M4 를 메인 스레드에서 돌리면 화면이 주기마다 3~9초씩
  통째로 언다. M3(AX 워커 스레드)는 선택이 아니라 M4 의 선행조건이다.
- 잠정 대응: 폴링 간격을 **마지막 읽기 시간의 3배**로 잡고 그 값을 화면에 적는다.
  고정 숫자를 지키는 척하지 않는다. M3 뒤에는 얼지 않으므로 다시 낮출 수 있다.

읽기를 싸게 만드는 길은 따로 있다 — 행 수집 바닥값을 호출자가 낮출 수 있게 하는 것.
...
### 속도 — 병목은 행 개수가 아니라 AX 질의 횟수였다 (2026-09-16)

단계별로 재고 나서야 알았다. 그 전의 추측(행 예산 줄이기)은 엉뚱한 곳을 건드린 것이었다.

| 단계 | 전 | 후 |
|---|---:|---:|
| 입력창 찾기 | 수초 | 2ms |
| 전사 루트 찾기 | **7,214ms** | 270ms |
| 행 모으기 | 3,966ms | 337ms |
| **방 열기 전체** | **7.2초** | **0.58초** |
| **읽기 전체** | 10~17초 | **4~6초** |

**카카오톡은 AX 질의 하나에 5~10ms 를 쓴다.** 그래서 `findAll(..., maxNodes: 600)` 한
번이 4~7초다. 비용은 노드 수에 정비례하고, 그것 말고는 아무것도 중요하지 않다.

처방은 세 군데 모두 같다: **실제로 있는 곳만 본다.** 입력창도 전사 컨테이너도 창의
직계 자식이거나 그 자식이다. 두 단계만 훑으면 수십 번의 질의로 끝나고, 넓이우선 탐색은
전사 말풍선 수백 개를 지나가느라 예산을 태운다.

남은 비용은 `extractMessages` — 행마다 정적 텍스트를 다시 훑는다. 같은 처방이 통할
자리지만 아직 안 건드렸다.

### A2 실측 — 가려진 창도 읽힌다 (2026-09-16)

`kbbs probe-send "…" --minimized` 로 확인. 창을 최소화하고 다시 읽었다.

```
최소화 전  10개 읽음
최소화 후  10개 읽음
최소화 상태 주입  반영됨 · 전송버튼 켜짐
```

**최소화된 창은 AX 로 그대로 읽히고, 입력창 주입과 전송 버튼 활성화까지 된다.**
설계는 반대로 가정하고 그 위에 여러 결정을 얹었다.

따라서 kbbs 가 열어야 했던 창은 **열자마자 최소화한다.** 화면을 뺏기는 시간은 1초 남짓이고
그 뒤로는 창이 쌓이지 않는다. 다시 보고 싶으면 `S`, 닫으려면 `W`.

### 행 핸들은 재사용된다 — 창 열기가 엉뚱한 방을 열던 이유 (2026-09-16)

증상 둘이 같은 원인이었다. "창 열기를 하면 자꾸 같은 방만 열린다", 그리고 "대화 내용이
갱신되지 않는다". 후자는 전자의 결과다 — 다른 방이 열렸으니 고른 방에서는 아무 일도
일어나지 않는다.

**AppKit 테이블은 행 뷰를 재사용한다.** 대화 목록은 메시지가 올 때마다 순서가 바뀌고,
그때 `ChatListScanner` 가 넘겨준 행 `AXUIElement` 는 **다른 대화를 표시하게 된다.** 핸들은
살아 있고 프레임도 멀쩡히 읽히므로 좌표 가드도 통과한다. 누르면 그 자리에 지금 있는 방이
열린다.

→ 누르기 직전에 행이 아직 그 제목을 표시하는지 읽어 확인하고, 아니면 다시 스캔한다.

**같이 확인했고 원인이 아니었던 것들** (전부 실측):

- 스크롤 위치: 위로 3쪽 올린 상태에서도 새 메시지가 AX 에 보인다.
- 최소화: 최소화해도 제목을 유지하고, 제목으로 찾히고, **최소화된 동안 도착한 메시지도
  보인다.** 그래서 창을 열자마자 숨기는 동작은 안전하다.

### 가리기는 최소화와 다르고, 모든 면에서 낫다 (2026-09-17)

`kbbs probe-hide` 로 실측. 앱 요소의 `AXHidden` — ⌘H 의 AX 철자 — 은 한 번도 시도된 적이
없었다. Worker 의 주석은 "앱 가리기는 안 된다" 고 단정하고 있었지만, 그건
`NSRunningApplication.hide()` 얘기였다.

| | 최소화 | 가리기 (`AXHidden`) |
|---|---|---|
| 걸리는 시간 | 550~830ms, 재시도 2~3회 | **25ms, 한 번** |
| 범위 | 창 하나씩 | 앱 전체 한 번에 |
| 그 상태에서 행 좌표 | **없음** | **그대로** |
| 대화 목록 읽기 | 됨 | 됨 (60행, 좌표 포함) |
| 입력창 값 주입 | 됨 | **됨** |
| 전송 버튼 활성 | 됨 | **됨** |

**최소화는 창을 Dock 으로 옮기므로 좌표가 사라진다. 가리기는 제자리에 둔 채 안 보이게만
한다.** 그래서 가린 상태에서 스캔한 행 좌표를 보이게 한 뒤 그대로 쓸 수 있다 — 창을
복원하고 다시 그려질 때까지 기다렸다가 재스캔하는 구간 전체가 없어진다.

`AXHidden` 쓰기도 다른 설정 가능 속성들과 같은 거짓말을 한다: 성공을 반환하고, **직후에
읽으면 옛날 값이 나온다.** 25ms 쯤 뒤에 반영된다. 그래서 쓰고 나서 확인될 때까지 읽어야
한다.

### 카카오톡은 구현하지 않은 AX 액션을 광고한다 (2026-09-16)

하루에 세 번 같은 함정에 빠졌다. 액션 이름이 `actionNames()` 에 나온다고 해서 그 액션이
동작하는 것은 아니다.

| 요소 | 광고하는 액션 | 실제 |
|---|---|---|
| 대화 목록 행 | (없음 — `AXShowMenu` 뿐) | 더블클릭만 가능 |
| 「전송」 버튼 | `AXPress` | **보낸다. 그리고 실패를 반환한다** |
| 목록 스크롤 영역 | `AXScrollDownByPage` 외 3개 | `Attribute unsupported` |
| 행 | `AXScrollToVisible` | 아무 일도 안 일어남 |

**→ 이 저장소에서 AX 액션은 전부 "해보고 결과를 확인"한다. 반환값도, 액션 목록도 증거가
아니다.**

### 창 열기가 아래쪽 방에서만 실패하던 이유 (2026-09-16)

목록은 스크롤되는 표라서 **화면 밖으로 밀려난 행도 프레임을 갖는다.** 창이 y 667~1307
인데 여덟 번째 행은 y=1280~1354 였고, 중심 y=1317 은 창 **아래 10px 바깥**이다. 좌표
가드는 "화면 안인가"만 봤으므로 통과했고, 클릭은 바탕화면에 떨어졌다.

위쪽 방(하나카드, y=1058)은 창 안이라 잘 열렸다. 그래서 "가끔 된다"로 보였다.

→ 가드에 **목록 창 경계**를 넣었다. 창 밖이면 먼저 휠 이벤트로 스크롤해 행을 창 안으로
들인 뒤, 프레임을 다시 읽고 누른다. 휠을 쓰는 이유는 위 표대로 스크롤 액션이 거짓이기
때문이다.

### 발신자 판정은 중심이 아니라 가장자리다 (2026-09-16)

사용자가 쓴 긴 메시지가 상대방 이름으로 표시됐다. 채팅 앱에서 낼 수 있는 최악의 오류다.

`inferMessageSide` 는 말풍선 **중심**의 비율로 좌우를 갈랐다 (`ratio <= 0.56` 이면 왼쪽).
말풍선이 폭을 거의 채우면 중심은 정렬과 무관하게 한가운데로 간다.

실측 (창 380 폭):

| | 왼쪽 여백 | 오른쪽 여백 |
|---|---:|---:|
| 상대 메시지 | **항상 60** | 138 ~ 186 |
| 내 메시지 | 80 ~ 323 | **항상 35** |
| 시스템/날짜 | 126 | 143 |

문제의 메시지는 `left=80 right=35` 였다. 오른쪽 여백 35 는 다른 모든 내 메시지와 같은
값인데, 중심 비율은 0.559 로 나와 임계값 0.56 에 **0.001 차이로** 걸렸다.

→ 가장자리 간격을 비교한다. 더 좁은 쪽이 정렬 방향이고, 차이가 폭의 8%(30pt) 미만이면
`unknown` 이다. 실측에서 진짜 차이는 45, 시스템 메시지는 17 이라 그 사이에 선이 있다.
`unknown` 은 화면에 `나?` 로 나온다 — 모르는 것을 아는 척하지 않는다.

### 숫자 방 이름을 안읽음 수로 오인 (2026-09-21)

사용자가 제공한 `kbbs inspect --depth 5` 덤프에서 문제 방의 제목은
`AXStaticText id=_NS:40 value="16"` 이고, 별도 `Count Label value="2"` 가 있었다.
`01`, `09`, `18` 등 다른 숫자 이름도 같은 제목 식별자를 쓴다.

기존 제목 추출은 숫자 모양을 안읽음 수로 제외하여 `(Unknown Chat)`을 만들었다.
기존 `[open]` 로그에서는 사전 스캔이 그 임시 제목으로 행을 찾지만, 클릭 직전
실제 제목 대조에서 `실패: 행 없음`으로 끝났다. 제목 식별자 `_NS:40`을 모양보다
우선하고, 안읽음 수의 폴백에서는 제목·시각 식별자를 제외하도록 수정했다.
클릭 직전 제목 대조와 탐색 예산은 유지했다.

회귀 테스트에서 숫자 제목 누락·안읽음 수 오인 6개 assertion 실패를 확인한 뒤,
수정 후 전체 360개 테스트와 `make lint-print`가 통과했다. 이번 근거는 사용자 제공
AX 덤프·기존 로그·단위 테스트다. 이후 `~/bin/kbbs`에 설치한 수정본에 대해 사용자가
"잘되네"로 보고한 증상의 해결을 확인하고 배포를 요청했다. 메시지 전송은 검증하지 않았다.

### 맥 잠금 뒤 목록 재시도가 멈춤 (2026-09-21)

사용자가 맥 잠금을 해제한 뒤 카카오톡은 실행 중인데도 `통신 두절` 화면에 머무는
현상을 보고했다. 화면은 두절 3780초, 다음 시도 10초를 표시했다.

목록 읽기가 3회 실패하면 blocked로 전환되지만, 자동 재시도와 R 처리에는 대화방
token이 필요했다. 목록에는 token이 없어 실제 재시도를 하지 않았다. 목록 성공
결과도 blocked 해제·연속 실패 횟수 초기화가 없었고, 재시도 초는 고정값이었다.

- blocked에서는 목록 스캔 또는 현재 방 읽기를 다시 요청한다. 목록 읽기 성공은
  정상 목록으로 복귀하고 연속 실패 횟수·마지막 정상 수신 시각을 갱신한다.
- 카운트다운은 실제 다음 시각으로 계산한다. 작업 중에는 중복 재시도하지 않는다.
- 워커는 읽기 실패 뒤 앱·창·대화 핸들을 버리고 다음 읽기에서 다시 연결한다.
  매 목록 스캔도 현재 목록 창을 새로 조회한다. 자동 실행·포커스 이동·전송 재시도는 없다.
- 대화방 복구는 기존 token으로 새 읽기 context를 연결하여 작성 중인 글을 보존한다.

실제 Loop에 워커 응답을 공급하는 회귀 테스트에서 11개 assertion 실패를 재현했다.
수정 후 복구 회귀 테스트 7개를 포함한 전체 367개 테스트와 lint가 통과했다.
이번에는 맥을 직접 잠그거나 메시지를 보내지 않았다. 실제 잠금/해제 후 복귀는
사용자 환경에서 추가 확인이 필요하다.

### 읽어 둔 대화의 화살표 탐색 (2026-09-22)

사용자 요청으로 원문 답장 기능을 원복한 뒤, 메시지 선택·이전 대화 보기만 복원했다.
↑ 첫 입력은 마지막 메시지를 선택하고 이후 ↑↓로 한 메시지씩 이동한다.
첫 메시지에서는 ↑가 멈추고, 마지막에서 ↓를 한 번 더 누르면 선택바가 사라진다.
선택 없는 상태의 ↓는 아무 동작도 하지 않는다. Esc는 선택 해제 후 다시 누르면
목록으로 복귀한다. 초안은 선택·해제로 바뀌지 않는다.

선택 중에는 읽어 둔 메시지 배열을 고정해 새 조회 결과로 선택 대상이 바뀌지 않게
하고, 선택을 해제하면 최신 조회 결과를 표시한다. 긴 한글 메시지도 24×80 프레임을
유지하며 선택한 메시지의 시작을 보여준다. 현재 불러온 범위 안에서만 탐색한다.
화살표 탐색으로 AX 작업을 추가 요청하지 않는다. Enter는 기존 일반 전송 동작이며
원문 답장 준비·메뉴 조작 코드는 복원하지 않았다.

검증: 실제 Loop 키 처리·선택 경계·새 조회 결과·Esc/초안·일반 전송 유지·한글 프레임
회귀 테스트 8개와 전체 XCTest 375개 통과. lint-print clean, diff 공백 검사 통과.
이번 변경에서 카카오톡 조작·실제 전송은 수행하지 않았다.

### 아직 미검증

A1(AXConfirm)·A2(가려진 창)·A3(따뜻한 읽기 지연) 셋 다 그대로. M1 실물 검증이
"카카오톡 창이 하나도 안 열려 있음" 때문에 막혀서, 진짜 대화방 목록이 화면에
뜨는 것은 아직 못 봤다.
...
| 마일스톤 | 상태 |
|---|---|
| M0 개발 루프 (`make install`, 고정 서명) | 완료 |
| M0 스파이크 (A1/A2/A3 측정) | **생략** — 사용자가 최악 전제 선택 |
| M1 목록 화면 | 코드 완료, **실물 검증 대기** (카카오톡 창 필요) |
| M2 원시 모드 + 키 입력 | 미착수 |
| M3 AX 워커 스레드 | 미착수 |
| M4 대화 읽기 | 미착수 |
| M5 실패 화면 | 완료 (지연 등급 · 백오프 · 대기 화면 · 게이트 실패 · 통신두절) |
| M6 전송 | 완료 (AXPress, 전사 대조로 확인) |
| M7 장시간 안정성 | 미착수 |
| M7 장시간 안정성 | 부분 (테스트 80개, lint-print) |
